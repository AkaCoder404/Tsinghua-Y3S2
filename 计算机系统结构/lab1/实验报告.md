...
Use an array space that is twice the size of L1 cache (256KB ```int[1024 * 64]```), divide it into $2^n$blocks, and access the odd block in turn. When access slows down, the cache association degree is $2^{n-2}$

Proof : 

Assuming that the associativity is $2^k$, the block access time is $2^m$ blocks, and the cache capcity is $C$, the cache row size is $L$, then the size of each cahce group is $C/2^k$, containing $C/2^kL$ cache rows, and the memory size of each block is $C/2^{M-1}$, including $C/2^{M-1}L$ cache rows.
